...
- (void)addRightUtilityButtonWithPredefined:(UIButton *)button {
    [((NSMutableArray *)self.scrollViewButtonViewRight.utilityButtons) addUtilityButtonWithPredefined:button];
    [self.scrollViewButtonViewRight populateUtilityButtons];
    self.rightUtilityButtons = self.scrollViewButtonViewRight.utilityButtons;

//    if (!self.scrollViewButtonViewRight) {
//        self.scrollViewButtonViewRight = [[SWUtilityButtonView alloc] initWithUtilityButtons:_rightUtilityButtons parentCell:self utilityButtonSelector:@selector(rightUtilityButtonHandler:)];
//        [self.scrollViewButtonViewRight setFrame:CGRectMake(CGRectGetWidth(self.bounds), 0, [self rightUtilityButtonsWidth], _height)];
//        [self.cellScrollView addSubview:self.scrollViewButtonViewRight];
//    }
    [self setNeedsLayout];
}

- (void)resetRightUtilityButtons
{
    [((NSMutableArray *) self.scrollViewButtonViewRight.utilityButtons) removeAllObjects];
    [((NSMutableArray *) self.rightUtilityButtons) removeAllObjects];
//    [self.scrollViewButtonViewRight removeFromSuperview];
...
- (void)hideUtilityButtonsAnimated:(BOOL)animated {
    ...
    [_delegate swippableTableViewCell:self didUtilityButtonVisiblilityChanged:NO];
}
...
- (void)scrollToRight:(inout CGPoint *)targetContentOffset{
    ...
    if ([_delegate respondsToSelector:@selector(swippableTableViewCell:didUtilityButtonVisiblilityChanged:)]) {
        [_delegate swippableTableViewCell:self didUtilityButtonVisiblilityChanged:YES];
    }
}
...
- (void)addUtilityButtonWithPredefined:(UIControl *)button {
    [self addObject:button];
}
